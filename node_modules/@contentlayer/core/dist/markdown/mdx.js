...
export const bundleMDX = ({ mdxString, options, contentDirPath, rawDocumentData, }) => (OT.withSpan('@contentlayer/core/markdown:bundleMDX')(T.tapError_(T.mapError_(T.gen(function* ($) {
    // TODO should be fixed in `mdx-bundler`
    if (mdxString.length === 0) {
        return '';
    }
    const { rehypePlugins, remarkPlugins, resolveCwd, cwd: cwd_, mdxOptions: mapMdxOptions, ...restOptions } = options ?? {};
    ...
    const res = yield* $(T.tryPromise(() => mdxBundler.bundleMDX({ source: mdxString, ...mdxOptions }), fileName_1 + ":67:40"), fileName_1 + ":67:27");
    if (res.errors.length > 0) {
        return yield* $(T.fail(res.errors, fileName_1 + ":70:31"), fileName_1 + ":70:24");
    }
    return res.code;
}, fileName_1 + ":24:10"), (error) => new UnexpectedMDXError({ error }), fileName_1 + ":75:15"), () => OT.addAttribute('mdxString', mdxString), fileName_1 + ":76:15")));
